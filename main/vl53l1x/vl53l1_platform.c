...
int _I2CWrite(VL53L1_DEV Dev, uint8_t *pdata, uint32_t count) {
    ...
    status = i2c_master_write_to_device(Dev->i2c_num, Dev->I2cDevAddr, pdata, count, pdMS_TO_TICKS(i2c_time_out));
    if (status != ESP_OK) {
        ESP_LOGE(TAG, "write i2c error, i2c port: %d, addr: 0x%x,  %s", Dev->i2c_num, Dev->I2cDevAddr, esp_err_to_name(status));
...
int _I2CRead(VL53L1_DEV Dev, uint8_t *pdata, uint32_t count) {
    ...
    //status =   HAL_I2C_Master_Receive(Dev->I2cHandle, Dev->I2cDevAddr|I2C_MASTER_READ, pdata, count, i2c_time_out);
    status = i2c_master_read_from_device(Dev->i2c_num, Dev->I2cDevAddr, pdata, count, pdMS_TO_TICKS(i2c_time_out));
    if (status) {
        ESP_LOGE(TAG, "read i2c error, i2c port: %d, addr: 0x%x,  %s", Dev->i2c_num, Dev->I2cDevAddr, esp_err_to_name(status));
        //XNUCLEO6180XA1_I2C1_Init(&hi2c1);
    }
    return status;
}
...
VL53L1_Error VL53L1_GetTickCount(
    ...
    *ptick_count_ms = pdTICKS_TO_MS(xTaskGetTickCount());
